...
function loadBrowsers($name, $mobile) {
  ...
  $result = mysql_queryf("SELECT useragents.engine as engine, useragents.name as name, useragents.os as os, (SELECT COUNT(*) FROM clients WHERE useragent_id=useragents.id AND updated > DATE_SUB(NOW(), INTERVAL 1 minute)) as clients, (engine=%s AND %s REGEXP version) as found FROM useragents WHERE active=1 AND mobile=%s ORDER BY engine, name;", $browser, $version, $mobile);
  ...
  while ( $row = mysql_fetch_array($result) ) {
    ...
    ?>
		<div class="browser<?php echo $row[0] != $engine ? " clear" : "";?><?php echo $row[3] ? " you" : "";?>">
			<img src="<?php echo swarmpath( "images/{$row[0]}.sm.{$os}png" ); ?>" class="browser-icon <?php echo $row[0]; ?>" alt="<?php echo $row[1]; ?>" title="<?php echo $row[1]; ?>"/>
			<span class="browser-name"><?php echo $num; ?></span>
			<?php if ( intval($row[3]) > 0 ) {
				echo '<span class="active">' . $row[3] . '</span>';
			}?>
		</div>
  <?php $engine = $row[0];
	}
